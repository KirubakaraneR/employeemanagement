...
<title>Assigned Employee Page</title>
<style>
  body {
    margin: 0;
    padding: 0;
  ...
  <div class="offset">
    <center>
      <table class="pure-table pure-table-horizontal">
        ...
        <tbody>
          <tr style="font-size: 20px;">
            <td><c:out value="${projectList.id}"/></td>
            <td><c:out value="${projectList.name}"/></td>
            <td><fmt:formatDate pattern="dd MMM,yyyy" 
                value="${projectList.startDate}"/></td>
            <td><fmt:formatDate pattern="dd MMM,yyyy" 
                value="${projectList.endDate}"/></td>
            <td><c:out value="${projectList.status}"/></td>
          </tr>
        </tbody>
      </table>
      </center><br>
      <center>
        ...
        <tbody>
          <form action="ProjectController" method="post">
            <c:forEach items="${projectList.getEmployees()}" var="employee"> 
              <tr style="font-size: 20px;">
                <td><c:out value="${employee.id}"/></td>
                <td><c:out value="${employee.name}"/></td>
                <td><c:out value="${employee.mobileNumber}"/></td>
                <td><c:out value="${employee.mailId}"/></td>
                <td><center><input type="checkbox" name="employees" 
                    value="<c:out value="${employee.id}"/>"></center></td>
              </tr>
            </c:forEach>
          </tbody>
        </table>
        ...
        <input type="hidden" name="action" 
            value="unAssignEmployeesToProject">
        <input type="hidden" name="id" 
            value="<c:out value="${projectList.id}"/>">
